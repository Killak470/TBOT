...
const useAppStore = create<AppState>((set, get) => ({
  ...
  startBot: async (botId) => {
    try {
      ...
      set((state) => {
        const updatedBots = state.api.bots.map(bot => 
  ...
  createBot: async (config) => {
    try {
      ...
      set((state) => {
        const newBot = {
          ...config,
          id: `bot${state.api.bots.length + 1}`,
          isActive: false,
          profit: 0,
          trades: 0,
          lastActive: 'Never'
  ...
  deleteBot: async (botId) => {
    try {
      ...
      // For the demo, just update local state
      set((state) => {
        const updatedBots = state.api.bots.filter(bot => bot.id !== botId);
  ...
  syncServerTime: async () => {
    try {
      ...
      // Calculate the time offset (server time - local time)
      // const serverTime = response.data.serverTime;
      // const localTime = Date.now();
      // const offset = serverTime - localTime;
      
      // For the demo, we'll use a fake offset
      const offset = -150; // 150ms behind server time
